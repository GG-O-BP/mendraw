...
-type message(FVW) :: {message, FVW} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FVX, FVY) :: {continue,
        FVX,
        gleam@option:option(gleam@erlang@process:selector(FVY))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FVZ, FWA) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FVZ,
        gleam@erlang@process:selector(message(FWA)),
        gleam@otp@system:debug_state(),
        fun((FVZ, FWA) -> next(FVZ, FWA))}.

-type started(FWB) :: {started, gleam@erlang@process:pid_(), FWB}.

-opaque initialised(FWC, FWD, FWE) :: {initialised,
        FWC,
        gleam@option:option(gleam@erlang@process:selector(FWD)),
        FWE}.

-opaque builder(FWF, FWG, FWH) :: {builder,
        fun((gleam@erlang@process:subject(FWG)) -> {ok,
                initialised(FWF, FWG, FWH)} |
            {error, binary()}),
        integer(),
        fun((FWF, FWG) -> next(FWF, FWG)),
        gleam@option:option(gleam@erlang@process:name(FWG))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FWI) :: {ack, {ok, FWI} | {error, binary()}} |
...
-spec continue(FWN) -> next(FWN, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FWZ, FXA), gleam@erlang@process:selector(FXA)) -> next(FWZ, FXA).
...
-spec initialised(FXG) -> initialised(FXG, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FXL, any(), FXN),
    gleam@erlang@process:selector(FXR)
) -> initialised(FXL, FXR, FXN).
...
-spec returning(initialised(FXW, FXX, any()), FYC) -> initialised(FXW, FXX, FYC).
...
-spec new(FYG) -> builder(FYG, FYH, gleam@erlang@process:subject(FYH)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FYM)) -> {ok, initialised(FYO, FYM, FYP)} |
        {error, binary()})
) -> builder(FYO, FYM, FYP).
...
-spec on_message(builder(FYY, FYZ, FZA), fun((FYY, FYZ) -> next(FYY, FYZ))) -> builder(FYY, FYZ, FZA).
...
-spec named(builder(FZJ, FZK, FZL), gleam@erlang@process:name(FZK)) -> builder(FZJ, FZK, FZL).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FZY))) -> gleam@erlang@process:selector(message(FZY)).
...
-spec receive_message(self(any(), FZU)) -> message(FZU).
...
-spec initialise_actor(
    builder(any(), any(), GAR),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, GAR} | {error, binary()})
...
-spec start(builder(any(), any(), GBE)) -> {ok, started(GBE)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(GBL), GBL) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(GBN),
    integer(),
    fun((gleam@erlang@process:subject(GBP)) -> GBN)
) -> GBP.
