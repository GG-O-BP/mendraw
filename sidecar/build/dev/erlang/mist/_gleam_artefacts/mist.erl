...
-opaque next(ADWI, ADWJ) :: {continue,
        ADWI,
        gleam@option:option(gleam@erlang@process:selector(ADWJ))} |
    normal_stop |
...
-opaque builder(ADWK, ADWL) :: {builder,
        integer(),
        fun((gleam@http@request:request(ADWK)) -> gleam@http@response:response(ADWL)),
...
-type websocket_message(ADWM) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, ADWM}.
...
-spec continue(ADWN) -> next(ADWN, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\mist.gleam", 58).
-spec with_selector(next(ADWR, ADWS), gleam@erlang@process:selector(ADWS)) -> next(ADWR, ADWS).
...
-spec convert_next(next(ADXG, ADXH)) -> mist@internal@next:next(ADXG, ADXH).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(ADYI)) -> gleam@http@response:response(ADYK))
) -> builder(ADYI, ADYK).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(ADYO, ADYP), integer()) -> builder(ADYO, ADYP).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), ADYU),
    integer(),
    gleam@http@response:response(ADYU)
) -> builder(mist@internal@http:connection(), ADYU).
...
-spec bind(builder(ADZG, ADZH), binary()) -> builder(ADZG, ADZH).
...
-spec with_ipv6(builder(ADZM, ADZN)) -> builder(ADZM, ADZN).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ADZS, ADZT), binary(), binary()) -> builder(ADZS, ADZT).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AEAI)
) -> {ok, websocket_message(AEAI)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AEAO, websocket_message(AEAP), mist@internal@websocket:websocket_connection()) -> next(AEAO, AEAP)),
    fun((mist@internal@websocket:websocket_connection()) -> {AEAO,
        gleam@option:option(gleam@erlang@process:selector(AEAP))}),
    fun((AEAO) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(AEBD)) -> {ok,
            gleam@otp@actor:initialised(AEBF, AEBD, any())} |
        {error, binary()}),
    fun((AEBF, AEBD, s_s_e_connection()) -> gleam@otp@actor:next(AEBF, AEBD))
) -> gleam@http@response:response(response_data()).
