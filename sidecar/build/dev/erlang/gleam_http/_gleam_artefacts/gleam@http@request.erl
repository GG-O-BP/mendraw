...
-spec set_header(request(FBL), binary(), binary()) -> request(FBL).
...
-spec prepend_header(request(FBO), binary(), binary()) -> request(FBO).
...
-spec set_body(request(any()), FBT) -> request(FBT).
set_body(Req, Body) ->
    {request,
...
-spec map(request(FBV), fun((FBV) -> FBX)) -> request(FBX).
...
-spec set_query(request(FCH), list({binary(), binary()})) -> request(FCH).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(FCL), gleam@http:method()) -> request(FCL).
...
-spec set_scheme(request(FCS), gleam@http:scheme()) -> request(FCS).
...
-spec set_host(request(FCV), binary()) -> request(FCV).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(FCY), integer()) -> request(FCY).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(FDB), binary()) -> request(FDB).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(FDE), binary(), binary()) -> request(FDE).
...
-spec remove_cookie(request(FDK), binary()) -> request(FDK).
