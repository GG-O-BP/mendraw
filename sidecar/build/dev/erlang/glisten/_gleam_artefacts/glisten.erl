...
-type message(YXM) :: {packet, bitstring()} | {user, YXM}.
...
-type connection(YXN) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(YXN))}.

-opaque next(YXO, YXP) :: {continue,
        YXO,
        gleam@option:option(gleam@erlang@process:selector(YXP))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(YXQ, YXR) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(YXR)) -> {YXQ,
            gleam@option:option(gleam@erlang@process:selector(YXR))}),
        fun((YXQ, message(YXR), connection(YXR)) -> next(YXQ, message(YXR))),
        gleam@option:option(fun((YXQ) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(YYR) -> next(YYR, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\glisten.gleam", 151).
-spec with_selector(next(YYV, YYW), gleam@erlang@process:selector(YYW)) -> next(YYV, YYW).
...
?DOC(false).
-spec convert_next(next(YZK, YZL)) -> glisten@internal@handler:next(YZK, YZL).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(YZQ, YZR), fun((YZR) -> YZU)) -> next(YZQ, YZU).
...
-spec map_user_selector(gleam@erlang@process:selector(message(YZX))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(YZX)).
...
-spec convert_loop(
    fun((AAAC, message(AAAD), connection(AAAD)) -> next(AAAC, message(AAAD)))
) -> fun((AAAC, glisten@internal@handler:loop_message(AAAD), glisten@internal@handler:connection(AAAD)) -> glisten@internal@handler:next(AAAC, glisten@internal@handler:loop_message(AAAD))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(AAAI)) -> {AAAK,
        gleam@option:option(gleam@erlang@process:selector(AAAI))})
) -> fun((glisten@internal@handler:connection(AAAI)) -> {AAAK,
    gleam@option:option(gleam@erlang@process:selector(AAAI))}).
...
-spec new(
    fun((connection(AAAQ)) -> {AAAS,
        gleam@option:option(gleam@erlang@process:selector(AAAQ))}),
    fun((AAAS, message(AAAQ), connection(AAAQ)) -> next(AAAS, message(AAAQ)))
) -> builder(AAAS, AAAQ).
new(On_init, Loop) ->
...
-spec with_close(builder(AAAZ, AABA), fun((AAAZ) -> nil)) -> builder(AAAZ, AABA).
...
-spec with_pool_size(builder(AABF, AABG), integer()) -> builder(AABF, AABG).
...
?DOC(false).
-spec with_http2(builder(AABL, AABM)) -> builder(AABL, AABM).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(AABX, AABY)) -> builder(AABX, AABY).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(AACD, AACE), binary(), binary()) -> builder(AACD, AACE).
...
-spec bind(builder(AABR, AABS), binary()) -> builder(AABR, AABS).
