...
-opaque supervisor(GRP, GRQ) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(GRP, GRQ))}.

-type message(GRR, GRS) :: any() | {gleam_phantom, GRR, GRS}.

-opaque builder(GRT, GRU) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((GRT) -> {ok, gleam@otp@actor:started(GRU)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(GRT, GRU)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(GRV, GRW) :: {local,
        gleam@erlang@process:name(message(GRV, GRW))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(GRX) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, GRX}.
...
-type erlang_child_spec_property(GRY, GRZ) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((GRY) -> {ok, gleam@otp@actor:started(GRZ)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(GSA, GSB))) -> supervisor(GSA, GSB).
get_by_name(Name) ->
...
-spec named(builder(GSR, GSS), gleam@erlang@process:name(message(GSR, GSS))) -> builder(GSR, GSS).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(GTA, GTB), integer(), integer()) -> builder(GTA, GTB).
...
-spec timeout(builder(GTG, GTH), integer()) -> builder(GTG, GTH).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(GTM, GTN), gleam@otp@supervision:restart()) -> builder(GTM, GTN).
...
-spec start(builder(GTS, GTT)) -> {ok,
        gleam@otp@actor:started(supervisor(GTS, GTT))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(GUR, GUS)) -> gleam@otp@supervision:child_specification(supervisor(GUR, GUS)).
supervised(Builder) ->
...
-spec start_child(supervisor(GUY, GUZ), GUY) -> {ok,
        gleam@otp@actor:started(GUZ)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GVW) -> {ok, gleam@otp@actor:started(GVX)} |
        {error, gleam@otp@actor:start_error()}),
    GVW
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GVX, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((GSH) -> {ok, gleam@otp@actor:started(GSI)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GSH, GSI).
...
-spec supervisor_child(
    fun((GSM) -> {ok, gleam@otp@actor:started(GSN)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GSM, GSN).
