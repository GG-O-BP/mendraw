...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DQE)) -> {ok, DQE} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DPY)) -> {ok, DPY} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DPU)) -> {ok, DPU} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DQK), fun((DQK) -> json())) -> json().
...
-spec array(list(DQO), fun((DQO) -> json())) -> json().
