...
-type message(YDL) :: {internal, internal_message()} | {user, YDL}.

-type loop_message(YDM) :: {packet, bitstring()} | {custom, YDM}.

-type loop_state(YDN, YDO) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(YDO)),
        glisten@transport:transport(),
        YDN}.

-type connection(YDP) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(YDP))}.

-type next(YDQ, YDR) :: {continue,
        YDQ,
        gleam@option:option(gleam@erlang@process:selector(YDR))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(YDS, YDT) :: {handler,
        glisten@socket:socket(),
        fun((YDS, loop_message(YDT), connection(YDT)) -> next(YDS, loop_message(YDT))),
        fun((connection(YDT)) -> {YDS,
            gleam@option:option(gleam@erlang@process:selector(YDT))}),
        gleam@option:option(fun((YDS) -> nil)),
        glisten@transport:transport()}.

-file("src\\glisten\\internal\\handler.gleam", 65).
?DOC(false).
-spec continue(YEG) -> next(YEG, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\glisten\\internal\\handler.gleam", 69).
?DOC(false).
-spec with_selector(next(YEK, YEL), gleam@erlang@process:selector(YEL)) -> next(YEK, YEL).
...
?DOC(false).
-spec start(handler(any(), YFA)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(YFA)))} |
