...
-type response(FLF) :: {response, integer(), list({binary(), binary()}), FLF}.
...
-spec set_header(response(FLU), binary(), binary()) -> response(FLU).
...
-spec prepend_header(response(FLX), binary(), binary()) -> response(FLX).
...
-spec set_body(response(any()), FMC) -> response(FMC).
...
-spec try_map(response(FLG), fun((FLG) -> {ok, FLI} | {error, FLJ})) -> {ok,
        response(FLI)} |
    {error, FLJ}.
...
-spec map(response(FME), fun((FME) -> FMG)) -> response(FMG).
...
-spec expire_cookie(response(FMP), binary(), gleam@http@cookie:attributes()) -> response(FMP).
