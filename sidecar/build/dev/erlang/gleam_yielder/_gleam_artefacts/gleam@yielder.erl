...
-type action(WLI) :: stop | {continue, WLI, fun(() -> action(WLI))}.

-opaque yielder(WLJ) :: {yielder, fun(() -> action(WLJ))}.

-type step(WLK, WLL) :: {next, WLK, WLL} | done.

-type chunk(WLM, WLN) :: {another_by,
        list(WLM),
        WLN,
        WLM,
        fun(() -> action(WLM))} |
...
-type sized_chunk(WLO) :: {another, list(WLO), fun(() -> action(WLO))} |
    {last, list(WLO)} |
    no_more.
...
-spec unfold_loop(WLW, fun((WLW) -> step(WLX, WLW))) -> fun(() -> action(WLX)).
...
-spec unfold(WLR, fun((WLR) -> step(WLS, WLR))) -> yielder(WLS).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> WMB)) -> yielder(WMB).
repeatedly(F) ->
...
-spec from_list(list(WMF)) -> yielder(WMF).
from_list(List) ->
...
-spec transform(yielder(WMP), WMR, fun((WMR, WMP) -> step(WMS, WMR))) -> yielder(WMS).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src\\gleam\\yielder.gleam", 204).
-spec fold_loop(fun(() -> action(WMZ)), fun((WNB, WMZ) -> WNB), WNB) -> WNB.
...
-spec fold(yielder(WMW), WMY, fun((WMY, WMW) -> WMY)) -> WMY.
...
-spec to_list(yielder(WNE)) -> list(WNE).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(WNH)) -> step(WNH, yielder(WNH)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(WNP)), integer()) -> fun(() -> action(WNP)).
...
-spec take(yielder(WNM), integer()) -> yielder(WNM).
...
-spec drop_loop(fun(() -> action(WNV)), integer()) -> action(WNV).
...
-spec map(yielder(WNY), fun((WNY) -> WOA)) -> yielder(WOA).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(WOG), yielder(WOI), fun((WOG, WOI) -> WOK)) -> yielder(WOK).
...
-spec append_loop(fun(() -> action(WOW)), fun(() -> action(WOW))) -> action(WOW).
...
-spec append(yielder(WOS), yielder(WOS)) -> yielder(WOS).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(WPE)))) -> action(WPE).
...
-spec flatten(yielder(yielder(WPA))) -> yielder(WPA).
flatten(Yielder) ->
...
-spec concat(list(yielder(WPI))) -> yielder(WPI).
concat(Yielders) ->
...
-spec flat_map(yielder(WPM), fun((WPM) -> yielder(WPO))) -> yielder(WPO).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(WPU)), fun((WPU) -> boolean())) -> action(WPU).
...
-spec filter(yielder(WPR), fun((WPR) -> boolean())) -> yielder(WPR).
...
-spec filter_map(yielder(WPX), fun((WPX) -> {ok, WPZ} | {error, any()})) -> yielder(WPZ).
...
-spec cycle(yielder(WQL)) -> yielder(WQL).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src\\gleam\\yielder.gleam", 709).
-spec find_loop(fun(() -> action(WQT)), fun((WQT) -> boolean())) -> {ok, WQT} |
    {error, nil}.
...
-spec find(yielder(WQP), fun((WQP) -> boolean())) -> {ok, WQP} | {error, nil}.
...
-spec find_map(yielder(WQX), fun((WQX) -> {ok, WQZ} | {error, any()})) -> {ok,
        WQZ} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(WRQ)), integer()) -> fun(() -> action({WRQ,
    integer()})).
...
-spec index(yielder(WRN)) -> yielder({WRN, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(WRT, fun((WRT) -> WRT)) -> yielder(WRT).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src\\gleam\\yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(WRY)), fun((WRY) -> boolean())) -> fun(() -> action(WRY)).
...
-spec take_while(yielder(WRV), fun((WRV) -> boolean())) -> yielder(WRV).
...
-spec drop_while_loop(fun(() -> action(WSE)), fun((WSE) -> boolean())) -> action(WSE).
...
-spec drop_while(yielder(WSB), fun((WSB) -> boolean())) -> yielder(WSB).
...
-spec scan_loop(fun(() -> action(WSL)), fun((WSN, WSL) -> WSN), WSN) -> fun(() -> action(WSN)).
...
-spec scan(yielder(WSH), WSJ, fun((WSJ, WSH) -> WSJ)) -> yielder(WSJ).
...
-spec zip_loop(fun(() -> action(WSU)), fun(() -> action(WSW))) -> fun(() -> action({WSU,
    WSW})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(WSP), yielder(WSR)) -> yielder({WSP, WSR}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src\\gleam\\yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(WTJ)), fun((WTJ) -> WTL), WTL, list(WTJ)) -> chunk(WTJ, WTL).
...
-spec chunk_loop(fun(() -> action(WTE)), fun((WTE) -> WTG), WTG, WTE) -> action(list(WTE)).
...
-spec chunk(yielder(WSZ), fun((WSZ) -> any())) -> yielder(list(WSZ)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(WTX)), integer(), list(WTX)) -> sized_chunk(WTX).
...
-spec sized_chunk_loop(fun(() -> action(WTT)), integer()) -> fun(() -> action(list(WTT))).
...
-spec sized_chunk(yielder(WTP), integer()) -> yielder(list(WTP)).
...
-spec intersperse_loop(fun(() -> action(WUE)), WUE) -> action(WUE).
...
-spec intersperse(yielder(WUB), WUB) -> yielder(WUB).
...
-spec any_loop(fun(() -> action(WUJ)), fun((WUJ) -> boolean())) -> boolean().
...
-spec any(yielder(WUH), fun((WUH) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src\\gleam\\yielder.gleam", 1228).
-spec all_loop(fun(() -> action(WUN)), fun((WUN) -> boolean())) -> boolean().
...
-spec all(yielder(WUL), fun((WUL) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src\\gleam\\yielder.gleam", 1273).
-spec update_group_with(WVD) -> fun((gleam@option:option(list(WVD))) -> list(WVD)).
...
-spec group_updater(fun((WUV) -> WUW)) -> fun((gleam@dict:dict(WUW, list(WUV)), WUV) -> gleam@dict:dict(WUW, list(WUV))).
group_updater(F) ->
...
-spec group(yielder(WUP), fun((WUP) -> WUR)) -> gleam@dict:dict(WUR, list(WUP)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(WVH), fun((WVH, WVH) -> WVH)) -> {ok, WVH} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(WVL)) -> {ok, WVL} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(WVT) -> yielder(WVT).
single(Elem) ->
    once(fun() -> Elem end).

-file("src\\gleam\\yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(WVZ)), fun(() -> action(WVZ))) -> action(WVZ).
...
-spec interleave(yielder(WVV), yielder(WVV)) -> yielder(WVV).
...
-spec fold_until_loop(
    fun(() -> action(WWH)),
    fun((WWJ, WWH) -> gleam@list:continue_or_stop(WWJ)),
    WWJ
) -> WWJ.
...
-spec fold_until(
    yielder(WWD),
    WWF,
    fun((WWF, WWD) -> gleam@list:continue_or_stop(WWF))
) -> WWF.
...
-spec try_fold(yielder(WWL), WWN, fun((WWN, WWL) -> {ok, WWN} | {error, WWO})) -> {ok,
        WWN} |
    {error, WWO}.
...
-spec first(yielder(WXB)) -> {ok, WXB} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(WXF), integer()) -> {ok, WXF} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(WXN), fun((WXN) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(WXQ, fun(() -> yielder(WXQ))) -> yielder(WXQ).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(WXT), WXT) -> yielder(WXT).
