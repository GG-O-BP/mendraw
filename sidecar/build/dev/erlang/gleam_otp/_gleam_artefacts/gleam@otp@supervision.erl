...
-type child_specification(GPB) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(GPB)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(GPC)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GPC).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(GPH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GPH).
supervisor(Start) ->
...
-spec significant(child_specification(GPM), boolean()) -> child_specification(GPM).
...
-spec timeout(child_specification(GPP), integer()) -> child_specification(GPP).
timeout(Child, Ms) ->
...
-spec restart(child_specification(GPS), restart()) -> child_specification(GPS).
...
-spec map_data(child_specification(GPV), fun((GPV) -> GPX)) -> child_specification(GPX).
