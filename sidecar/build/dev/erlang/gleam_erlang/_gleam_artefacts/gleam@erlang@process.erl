...
-opaque subject(EFJ) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EFJ)}.

-type name(EFK) :: any() | {gleam_phantom, EFK}.

-type do_not_leak() :: any().

-type selector(EFL) :: any() | {gleam_phantom, EFL}.
...
-spec 'receive'(subject(EGJ), integer()) -> {ok, EGJ} | {error, nil}.
...
-spec receive_forever(subject(EGR)) -> EGR.
...
-spec selector_receive(selector(EGV), integer()) -> {ok, EGV} | {error, nil}.
...
-spec selector_receive_forever(selector(EGZ)) -> EGZ.
...
-spec map_selector(selector(EHB), fun((EHB) -> EHD)) -> selector(EHD).
map_selector(A, B) ->
...
-spec merge_selector(selector(EHF), selector(EHF)) -> selector(EHF).
merge_selector(A, B) ->
...
-spec select_map(selector(EHQ), subject(EHS), fun((EHS) -> EHQ)) -> selector(EHQ).
...
-spec select(selector(EHM), subject(EHM)) -> selector(EHM).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EIA)
) -> selector(EIA).
...
-spec select_other(selector(EIE), fun((gleam@dynamic:dynamic_()) -> EIE)) -> selector(EIE).
...
-spec deselect(selector(EHV), subject(any())) -> selector(EHV).
...
-spec select_specific_monitor(selector(EIQ), monitor(), fun((down()) -> EIQ)) -> selector(EIQ).
...
-spec select_monitors(selector(EIT), fun((down()) -> EIT)) -> selector(EIT).
...
-spec select_trapped_exits(selector(EHJ), fun((exit_message()) -> EHJ)) -> selector(EHJ).
...
-spec deselect_specific_monitor(selector(EIW), monitor()) -> selector(EIW).
...
-spec send_after(subject(EJS), integer(), EJS) -> timer().
...
-spec perform_call(
    subject(EIZ),
    fun((subject(EJB)) -> EIZ),
    fun((selector(EJB)) -> {ok, EJB} | {error, nil})
) -> EJB.
...
-spec call(subject(EJG), integer(), fun((subject(EJI)) -> EJG)) -> EJI.
...
-spec call_forever(subject(EJK), fun((subject(EJM)) -> EJK)) -> EJM.
