...
-type valid_message(ADHY) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, ADHY}.

-type websocket_message(ADHZ) :: {valid, valid_message(ADHZ)} | invalid.
...
-type handler_message(ADIA) :: {internal, gramps@websocket:frame()} |
    {user, ADIA}.

-type websocket_state(ADIB) :: {websocket_state,
        bitstring(),
        ADIB,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(ADJH))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(ADJH))).
...
-spec apply_frames(
    ...
    fun((ADIX, handler_message(ADIY), websocket_connection()) -> mist@internal@next:next(ADIX, ADIY)),
    websocket_connection(),
    mist@internal@next:next(ADIX, websocket_message(ADIY)),
    fun((ADIX) -> nil)
) -> mist@internal@next:next(ADIX, websocket_message(ADIY)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {ADIK,
        gleam@option:option(gleam@erlang@process:selector(ADIL))}),
    fun((ADIK) -> nil),
    fun((ADIK, handler_message(ADIL), websocket_connection()) -> mist@internal@next:next(ADIK, ADIL)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(ADIL)))} |
    {error, nil}.
