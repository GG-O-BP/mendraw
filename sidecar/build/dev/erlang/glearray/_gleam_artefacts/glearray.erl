...
-type array(HQN) :: any() | {gleam_phantom, HQN}.
...
-spec from_list(list(HQQ)) -> array(HQQ).
from_list(List) ->
    erlang:list_to_tuple(List).

-file("src\\glearray.gleam", 44).
?DOC(" Converts an array to a list.\n").
-spec to_list(array(HQT)) -> list(HQT).
to_list(Array) ->
...
-spec get(array(HQY), integer()) -> {ok, HQY} | {error, nil}.
get(Array, Index) ->
...
-spec get_or_default(array(HRC), integer(), HRC) -> HRC.
...
-spec copy_set(array(HRG), integer(), HRG) -> {ok, array(HRG)} | {error, nil}.
...
-spec copy_push(array(HRQ), HRQ) -> array(HRQ).
...
-spec copy_insert(array(HRT), integer(), HRT) -> {ok, array(HRT)} | {error, nil}.
