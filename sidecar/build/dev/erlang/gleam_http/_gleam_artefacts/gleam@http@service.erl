...
-spec map_response_body(
    fun((FPB) -> gleam@http@response:response(FPC)),
    fun((FPC) -> FPE)
) -> fun((FPB) -> gleam@http@response:response(FPE)).
...
-spec prepend_response_header(
    fun((FPG) -> gleam@http@response:response(FPH)),
    binary(),
    binary()
) -> fun((FPG) -> gleam@http@response:response(FPH)).
...
-spec ensure_post(gleam@http@request:request(FPK)) -> {ok,
        gleam@http@request:request(FPK)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(FPT)) -> FPV)) -> fun((gleam@http@request:request(FPT)) -> FPV).
